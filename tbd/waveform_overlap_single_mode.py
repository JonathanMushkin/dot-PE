...
Uses a single mode (m=2), which allows for more efficient calculations.
TODO: results are not stable. get nan overlaps. Need to fix JM 2025-02-11
"""
...
def max_eig_grid(M: torch.Tensor, phi_grid: torch.Tensor) -> torch.Tensor:
    """
    Return the maximal eigenvalues of the matrix M * exp(1j * phi)
    for each phi in phi_grid.
    """
    m_r = M.real
    m_i = -M.imag
    mrmr = (m_r.transpose(-2, -1) @ m_r)[..., np.newaxis, :, :]
    mimi = (m_i.transpose(-2, -1) @ m_i)[..., np.newaxis, :, :]
    mimr = (m_i.transpose(-2, -1) @ m_r)[..., np.newaxis, :, :]
    mrmi = (m_r.transpose(-2, -1) @ m_i)[..., np.newaxis, :, :]
    c = torch.cos(phi_grid).reshape(-1, 1, 1)
    s = torch.sin(phi_grid).reshape(-1, 1, 1)
    mm = mrmr * c**2 + mimi * s**2 + (mimr + mrmi) * c * s
    t = torch.einsum("...ii->...", mm)
    d = torch.linalg.det(mm)
    max_eigenvalues = torch.sqrt(0.5 * (t + torch.sqrt(t**2 - 4 * d)))
...
def compute_and_save_hh(
    ...
    with open(bank_folder / "bank_config.json", "r", encoding="utf-8") as fp:
        ...
        m_arr = bank_config["m_arr"]

    hh_ipp_filepath = target_folder / "hh_ipp.npy"
    norm_i_filepath = target_folder / "norm_i.npy"
    if not overwrite and hh_ipp_filepath.exists() and norm_i_filepath.exists():
        shape = np.load(hh_ipp_filepath).shape
        if shape[0] == bank_size:
            print("Inner products already computed. Skipping.")
            return

    intrinsic_sample_processor = get_intrinsic_sample_processor(bank_folder)
    waveform_dir = bank_folder / "waveforms"
    weights = torch.as_tensor(get_weights(bank_folder), device=device)[:, 0, 0]
    ...
    for bi in tqdm(
        ...
        amp = torch.as_tensor(amp, device=device, dtype=wf_dtype)[:, 0, ...]
        phase = torch.as_tensor(phase, device=device, dtype=wf_dtype)[:, 0, ...]
        h_ipb = amp * torch.exp(1j * phase)
        del amp, phase
...
def get_optimal_overlap_ijtopp(
    h1h1_ipp: torch.Tensor,
    h2h2_jpp: torch.Tensor,
    h1h2_itjpp: torch.Tensor,  # complex
    n_phi: int = 32,
    clip: bool = True,
    ...
    h1h1_ipp: (n_i, n_p, n_p)
    h2h2_jpp: (n_j, n_p, n_p)
    h1h2_itjpp: (n_i, n_t, n_j, n_p, n_p)

    return
    max_overlap : (n_i, n_t, n_o, n_j)
    """
    wf_device = h1h2_itjpp.device
    wf_dtype = h1h2_itjpp.dtype.to_complex()
    ip_dtype = h1h1_ipp.dtype
    phi_grid = torch.linspace(0, 2 * np.pi, n_phi, device=wf_device, dtype=ip_dtype)
    if normalize:
        norm1_i = h1h1_ipp[..., 0, 0] + h1h1_ipp[..., 1, 1]  # i
        norm2_j = h2h2_jpp[..., 0, 0] + h2h2_jpp[..., 1, 1]  # o

        h1h1_ipp = h1h1_ipp / norm1_i[..., None, None]
        h2h2_jpp = h2h2_jpp / norm2_j[..., None, None]
        h1h2_itjpp = h1h2_itjpp / np.sqrt(
            flex_reshape(norm1_i, "io", "itojpP") * flex_reshape(norm2_j, "j", "itojpP")
        )

    eigvals1_ip, eigvecs1_ipp = eig_2by2(h1h1_ipp)  # (i,i,2) vals, (i,2) vecs
    eigvals2_jp, eigvecs2_jpp = eig_2by2(h2h2_jpp)  # (j,2) vals, (j,2) vecs

    d1_inv_ipp = torch.zeros_like(h1h1_ipp)
    d2_inv_jpp = torch.zeros_like(h2h2_jpp)
    d1_inv_ipp[..., 0, 0] = 1 / torch.sqrt(eigvals1_ip[..., 0])
    d1_inv_ipp[..., 1, 1] = 1 / torch.sqrt(eigvals1_ip[..., 1])
    ...
    real = h1h2_itjpp.real
    imag = h1h2_itjpp.imag
    leftmost_ipp = d1_inv_ipp @ eigvecs1_ipp.transpose(-1, -2)
    rightmost_jpp = eigvecs2_jpp @ d2_inv_jpp
    C_real_itjpp = real @ rightmost_jpp
    C_imag_itjpp = imag @ rightmost_jpp
    C_real_jtipp = torch.moveaxis(C_real_itjpp, (0, 1, 2, 3, 4), (2, 1, 0, 3, 4))
    C_imag_jtipp = torch.moveaxis(C_imag_itjpp, (0, 1, 2, 3, 4), (2, 1, 0, 3, 4))
    D_real_jtipp = leftmost_ipp @ C_real_jtipp
    D_imag_jtipp = leftmost_ipp @ C_imag_jtipp

    D_real_ijtpp = torch.moveaxis(D_real_jtipp, (0, 1, 2, 3, 4), (1, 2, 0, 3, 4))
    D_imag_ijtpp = torch.moveaxis(D_imag_jtipp, (0, 1, 2, 3, 4), (1, 2, 0, 3, 4))
    D_ijtpp = D_real_ijtpp + 1j * D_imag_ijtpp
    max_overlap_ijto = max_eig_grid(
        D_ijtpp,
        phi_grid,
    )
    if clip:
        max_overlap_ijto = max_overlap_ijto.clip(max=1.0)
    return max_overlap_ijto
...
def get_inner_product_matrix(
    ...
    ----------
    weights : array-like
        Weight matrix for the inner product. Shape: dpPb
    h1 : array-like
...
def apply_time_shift(bank_folder, h, t):
    """
    ...
    fbin = fbin.reshape(-1, 1)  # Shape (..., n_b, 1)

    # Reshape t and phi for broadcasting
    t = t[None, :]  # Shape (n_t, 1)

    # Calculate shifts with broadcasting
    timeshift = torch.exp(-2j * np.pi * fbin * t)  # Shape (n_b, n_t)

    # Apply shifts to h and return
    h_shifted = h[..., None] * timeshift
    return h_shifted


def get_inner_product_matrix_flexible(
    w: torch.Tensor, h1: torch.Tensor, h2: torch.Tensor, complex: bool = False
):
    """
    ...
    h1_extra_dims = "".join(
        chr(ord("i") + i) for i in range(h1.ndim - len("pb"))
    )  # 'i', 'j', ...
    h2_extra_dims = "".join(
        chr(ord("J") + i) for i in range(h2.ndim - len("Pb"))
    )  # 'J', 'K', ...
    w_dims = "dpPb"
    h1_dims = h1_extra_dims + "pb"
    h2_dims = h2_extra_dims + "Pb"
    result_dims = h1_extra_dims + h2_extra_dims + "pP"
    einsum_expr = f"{w_dims}, {h1_dims}, {h2_dims} -> {result_dims}"
    res = torch.einsum(einsum_expr, w, h1, h2.conj())
    if complex is False:
        res = res.real
    return res
...
def _calculate_overlap_matrix_by_subbanks(
    ...
    def complex_ip_tensor(x):
        complex_dtype = torch.promote_types(ip_dtype, torch.complex64)
        if not isinstance(x, torch.Tensor):
            return torch.tensor(x, device=ip_device, dtype=complex_dtype)
        return safe_move_and_cast(x, dtype=complex_dtype, device=ip_device)
    ...
    t_grid = (torch.arange(n_t, dtype=wf_dtype, device=wf_device) - n_t // 2) * dt
    ...
    for sb, sub_bank in enumerate(sub_banks[sb_start:], start=sb_start):
        ...
        for m in tqdm(
            ...
            amp = wf_tensor(amp[:, 0, ...])
            phase = wf_tensor(phase[:, 0, ...])
            h1_ipb = amp * torch.exp(1j * phase)
            norm_i = ip_tensor(
                np.load(target_folder / "norm_i.npy")[i_bank_inds],
            )
            h1_ipbt = apply_time_shift(bank_folder, h1_ipb, t_grid)

            temp_ipbt = torch.sqrt(flex_reshape(norm_i, "i", "ipbt"))
            h1_ipbt = h1_ipbt / wf_tensor(temp_ipbt)
            del h1_ipb, amp, phase
            h1_itpb = torch.moveaxis(h1_ipbt, (0, 1, 2, 3), (0, 2, 3, 1))

            h1h1_ipp = ip_tensor(
                np.load(target_folder / "hh_ipp.npy")[i_bank_inds],
            )
            ...
            for n in tqdm(range(num_n_blocks), total=num_n_blocks, leave=False):
                ...
                amp = wf_tensor(amp[:, 0, ...])
                phase = wf_tensor(phase[:, 0, ...])
                h2_jpb = amp * torch.exp(1j * phase)
                norm_j = ip_tensor(
                    np.load(target_folder / "norm_i.npy")[j_abs_inds],
                )
                h2_jpb /= wf_tensor(torch.sqrt(flex_reshape(norm_j, "j", "jpb")))
                h2h2_jpp = ip_tensor(
                    np.load(
                        target_folder / "hh_ipp.npy",
                    )[j_abs_inds],
                )

                h1h2_itjpp = get_inner_product_matrix_flexible(
                    weights, h1_itpb, h2_jpb, complex=True
                )
                h1h2_itjpp = complex_ip_tensor(h1h2_itjpp)
...
def profile_and_run(**kwargs):
    ...
    target_folder = Path(kwargs["target_folder"]) if kwargs["target_folder"] else None
    if target_folder is None:
        target_folder = bank_folder / "overlaps"
