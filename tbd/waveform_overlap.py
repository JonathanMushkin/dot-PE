...
def compute_and_save_hh(
    ...
    for bi in tqdm(
        ...
        # TODO: REPLACE get_inner_product_matrix WITH RESHAPE + MATMUL CALCULATION
        # SOMETHING OF THE FORM
        # A = h_impb.reshape(...)
        # C = weights.reshape(...)
        # result = torch.sum(A.unsqueeze(2) * A.unsqueeze(1) * C, dim=(-1, -2, -3))
        # proboably can squeeze more out of it using matmul and
        # splitting to real and imaginary parts
...
def _calculate_overlap_matrix_by_subbanks(
    ...
    phi_grid = torch.linspace(0, 2 * np.pi, n_phi, device=wf_device, dtype=wf_dtype)
    ...
    for sb, sub_bank in enumerate(sub_banks[sb_start:], start=sb_start):
        ...
        for m in tqdm(
            ...
            h1_itompb = apply_time_and_phase_shift(
                bank_folder, h1_impb, t_grid, phi_grid
            ) / wf_tensor(torch.sqrt(flex_reshape(norm_io, "io", "itompb")))
            del h1_impb, amp, phase
            ...
            for n in tqdm(range(num_n_blocks), total=num_n_blocks, leave=False):
                ...
                h1h2_itojpp = ip_tensor(
                    get_inner_product_matrix_flexible(
                        weights,
                        h1_itompb,
                        h2_jmpb,
                    )
