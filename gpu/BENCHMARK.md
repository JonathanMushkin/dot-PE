...
CPU incoherent rate: ~2.9s/batch (512 batches x 2048 samples). GPU: ~0.81s/batch -- 3.6x
faster. At 1M scale incoherent dominates (~85% of CPU time), giving 2.7x end-to-end
speedup vs 1.8x at 128k. Coherent also faster on GPU (1.4x) at this scale.
...
- After Phase F filter skip (merged from main): 114s (GPU) / 111s (CPU) -- **6x improvement**
