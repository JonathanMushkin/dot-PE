...
Same 3 intrinsic indices selected: [495, 975, 1200] (ok)
...
|------|-------------|-------------|---------|
| `get_response_over_distance_and_lnlike` (48 calls) | 30.1s | 2.74s | **11x** |
| `select_intrinsic_samples_per_bank_incoherently` | 47.9s | 18.5s | **2.6x** |
| `get_dh_by_mode` (1 call, tiny block) | 20ms | 15ms | ~1.3x |
| `get_hh_by_mode` (1 call, tiny block) | 7ms | 13ms | overhead |
| **Total inference.run()** | 254s | 225s | 1.13x |
...
- Algorithm:
  - Per-sample focused grid in u-space (+/-10sigma around likelihood peak) + shared broad grid
...
- `test_sample_distance_batched` (new): N=2000 samples, compares mean/std vs CPU within 10% statistical tolerance
  - CPU mean=1261.8 Mpc, GPU mean=1243.0 Mpc (ok)
  - CPU std=3677.8 Mpc, GPU std=3693.1 Mpc (ok)
...
|------|-----|------------|---------|
| `select_intrinsic_samples_per_bank_incoherently` | 47.9s | 18.6s | **2.6x** |
| Coherent blocks (1 block, 25x512) | ~50s est. | 2.18s | **>23x** |
| `standardize_samples` (dist + prior, wall) | ~146s | ~12s wall / 5.4s CPU | **>12x** |
| `draw_extrinsic_samples` | ~51s | 51.0s | 1.0x (locked) (CPU-bound QMC) |
| **Total `inference.run()`** | **254s** | **~111s** | **~2.3x** |

### Current bottleneck profile (GPU run_12)
1. `draw_extrinsic_samples` -- **51.0s** (QMC sampling + marginalization, CPU-bound, (locked) cogwheel internals)
   - Inside: `marginalized_extrinsic.__init__` (41.8s), scipy sparse CSR matvec (6.3s), `optimize_beta_temperature` (15.3s)
2. `select_intrinsic_samples_per_bank_incoherently` -- **18.6s** (GPU, 48 batches x 0.39s, mainly waveform I/O)
...
- `standardize_samples` wall: 34.2s -> 31.5s -> vs CPU ~146s -> **>4.6x speedup**
- `inference.run()` total: 144.9s -> ~111s -> vs CPU 254s -> **~2.3x overall speedup**
...
132k x `SimInspiralTransformPrecessingWvf2PE` calls = 3.8s cProfile / ~13s wall.
...
**Python overhead dominates**: 10us/call x 1,638,400 calls = 16.4s (not LAL math itself).
...
|------|-----|------------|------------|-----------------|
| `select_intrinsic_samples` | 47.9s | 18.6s | 18.7s | **2.6x** |
| Coherent blocks (25x512) | ~50s est. | 2.18s | 2.17s | **>23x** |
| `standardize_samples` (wall) | ~146s | ~31.5s | **1.06s** | **>137x** |
| `draw_extrinsic_samples` | ~51s | 51.0s | 50.7s | 1.0x (locked) |
| **Total `inference.run()`** | **254s** | **~111s** | **~81s** | **~3.1x** |

**Standardize speedup**: 146s -> 1.06s = **>137x** (all subpriors now vectorized).
**Remaining bottlenecks**:
1. `draw_extrinsic_samples`: 50.7s ((locked) cogwheel QMC, locked)
2. `select_intrinsic_samples`: 18.7s (GPU, 2.6x, mainly waveform I/O)
...
- draw_extrinsic: ~514s (QMC on 5798 intrinsic samples, (locked) locked)
...
### Setup
- **Bank**: `test_bank_1048576` (1,048,576 templates), q in [0.25,1], mchirp in [10,40]
...
GPU speedup improves at 1M bank (3.27x) vs 32k bank (2.6x) because:
...
|-----------|----------------------|-----|---------|
| 32k (2^15) | 18.7s (16 batches) | 47.9s | **2.6x** |
| 1M (2^20) | 405.4s (512 batches) | 1323.7s | **3.3x** |
...
- `load_amp_and_phase`: 424.9s cumtime (514 calls x 0.83s)
- `get_relative_linfree_dt_from_waveform`: 327.7s cumtime (514 calls x 0.64s)
...
**Memory**: 1M x 4 modes x 2 pol x 378 freq bins x complex64 ~ **24 GB** (fits in 44 GB L40S VRAM).
...
### Impact
- draw_extrinsic: **692s -> 114s** (6x improvement) -- dominant fix for this stage
...
### nb04 -- 2 x 64k banks (128k total)

| Stage | CPU | GPU | Speedup |
|---|---|---|---|
| Incoherent bank_0 (65k) | 150 | 27 | 5.6x |
| Incoherent bank_1 (65k) | 96 | 27 | 3.6x |
| draw_extrinsic | 111 | 119 | ~1x |
| Coherent (both banks) | 79 | 72 | ~1x |
...
| Stage | CPU | GPU | Speedup |
|---|---|---|---|
| Incoherent (1M) | 1490 | 417 | 3.6x |
| draw_extrinsic | 106 | 112 | ~1x |
| Coherent (20 blocks) | 162 | 115 | 1.4x |
| Standardizing | 2 | 0.3 | -- |
| **Total** | **~1760** | **~644** | **2.7x** |

At 1M scale, incoherent dominates (~85% of CPU time) -> end-to-end speedup jumps to 2.7x
vs 1.8x at 128k. See `gpu/BENCHMARK.md` for full details.
