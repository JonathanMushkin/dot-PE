...
The CPU bottleneck at 2^18+ scale:
  ~10M calls × np.vectorize(lookup_table.sample_distance(..., resolution=32))
  Each call: build ~64-point distance grid, fit scipy spline, sample CDF.

GPU replacement:
  Chunked kernel over batches of CHUNK rows × K grid points:
...
Entry point: sample_distance_batched_gpu(d_h_arr, h_h_arr, lookup_table, resolution=32)
"""
...
from gpu.gpu_constants import DEVICE, REAL_DTYPE
...
def _sample_chunk(
    ...
    lookup_table,
    K: int,
    _u_fracs: np.ndarray = None,  # optional (C,) fractional quantiles in (0,1) for testing
) -> np.ndarray:
    ...
    delta_u = 10.0 / norm_h        # (C,)  10σ half-width

    # Mirror CPU grid construction exactly:
    #   CPU: linspace(u_peak+delta_u, u_peak-delta_u, K), then filter to u > 0 & d < D_MAX
    # GPU: same linspace spacing, but clip u to [REF/D_MAX, ∞) instead of filtering.
    # Points with u < REF/D_MAX map to d ≥ D_MAX where the posterior is ~0, so they
    # add negligible weight to the CDF while keeping K fixed for batching.
    ...
    # Focused grid on GPU: descending in u (matches CPU linspace direction),
    # clipped at u_min so d stays within [d_MIN, D_MAX].
    t_g     = torch.linspace(0.0, 1.0, K, device=DEVICE, dtype=F64)              # (K,)
    u_foc_g = u_hi_g.unsqueeze(1) - (u_hi_g - u_lo_g).unsqueeze(1) * t_g.unsqueeze(0)  # (C,K)
    ...
    d_broad_g = torch.linspace(D_MAX / K, D_MAX, K, device=DEVICE, dtype=F64)    # (K,)
    d_broad_g = d_broad_g.unsqueeze(0).expand(C, -1)                              # (C,K)

    # Combined grid (C, 2K), sorted per row
    d_g, _ = torch.sort(torch.cat([d_broad_g, d_foc_g], dim=1), dim=1)           # (C, 2K)
    ...
    u_g         = REF / d_safe                                             # (C, 2K)
    exponent    = -0.5 * (norm_h_g.unsqueeze(1) * u_g - overlap_g.unsqueeze(1)) ** 2
    posterior_g = prior_g * inv_vol * torch.exp(torch.clamp(exponent, min=-1000.0))
    posterior_g = posterior_g * (d_g > 0).double()

    # CDF via trapezoidal rule (float64 cumsum avoids precision loss in the tail)
    delta_d = d_g[:, 1:] - d_g[:, :-1]                                    # (C, 2K-1)
    trap    = 0.5 * (posterior_g[:, :-1] + posterior_g[:, 1:]) * delta_d  # (C, 2K-1)
    ...
    if _u_fracs is not None:
        u_samp = torch.from_numpy(_u_fracs).to(DEVICE, dtype=F64) * cdf_max
    else:
        u_samp = torch.rand(C, device=DEVICE, dtype=F64) * cdf_max        # (C,)

    # CDF inversion via searchsorted + linear interpolation.
    # searchsorted returns the INSERT position i s.t. cdf[i-1] < u_samp <= cdf[i],
    # so the lower bound for interpolation is idx = insert_pos - 1.
    idx   = torch.searchsorted(cdf_g.contiguous(), u_samp.unsqueeze(1)).squeeze(1) - 1
...
def sample_distance_batched_gpu(
    ...
    lookup_table,
    resolution: int = 32,
    _u_fracs: np.ndarray = None,  # (N,) fractional quantiles in (0,1) — for testing only
) -> np.ndarray:
    """
    Batched GPU replacement for ``lookup_table.sample_distance`` called over arrays.
    ...
    _u_fracs : (N,) float64, optional
        Fractional quantile values in (0, 1) to use instead of random draws.
